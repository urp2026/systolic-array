...
# 2. 16진수 문자열 리스트를 4x4 행렬로 변환하는 함수 (INT16 버전)
def hex_list_to_matrix(hex_list):
    
    # 16-bit 부호 있는 정수(2의 보수) 변환
    # 16-bit이기에 16진수 0x8000 ~ 0xFFFF는 음수가 아닌 양수로 인식됨 -> 코드상으로 이 수들을 음수로 바꿔줘야함
    # int(x,16) : x를 16진수로 인식해서 10진수로 출력
    # x가 32767보다 크다면 (>=0x8000) 65536(2^16)을 빼서 음수로 처리
    dec_list = [int(x, 16) - 65536 if int(x, 16) > 32767 else int(x, 16) for x in hex_list]
...
# with : python에서는 file을 open하고 close하지 않으면 오류 발생 -> with문법을 사용하면 이 행이 실행된 이후 바로 file이 close됨 
    ...
    # zip : fa, fb에 있는 각 줄을 묶어 각각 line_a, line_b로 전달함(같은 줄에 있는 것을 동시에 전달)
        ...
        # matrix multiply 수행
        C = np.matmul(A, B)
        print(f"=== Test Case {case_idx} (INT16) ===")
        print(C)
        print()
        
        # C의 각 원소는 결과적으로 16비트로 제한하여 출력합니다.
        # C.flatten() : matrix 형태의 C를 일자로 늘려서 저장
        # for문에서 val에 C의 각 원소가 들어감
        # val & 0xFFFF : 16bit로 val을 잘라냄 (부호 비트 포함 보존)
        # hex : 16진수로 변환
        # [2:] python에서 16진수로 변환시 생기는 0x를 잘라냄
        # .zfill(4) 길이가 4자리가 안된다면 앞에서부터 0으로 채움 (16비트 = Hex 4자리)
